...
import com.game.app.data.courses.CourseDialogPreferences
import com.game.app.databinding.FragmentCategoryDialogBinding
...
class CategoryDialogFragment : BaseFragment<CourseViewModel, FragmentCategoryDialogBinding, UserRepository>(){
    private lateinit var categoryHighAdapter: CategoryHighAdapterV
    private lateinit var coursePreferences: CourseDialogPreferences

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        // Инициализация внутреннего хранилища
        coursePreferences = CourseDialogPreferences(requireContext())

        categoryHighAdapter = CategoryHighAdapterV(requireContext(), categories = arrayListOf())

        // Добавление Item Decoration к RecyclerView
        // binding.recyclerViewDialog.addItemDecoration(ItemDecoration(800, 800))
        binding.recyclerViewDialog.setAdapter(
            requireContext(),
            categoryHighAdapter,
        ...
        viewModel.getCoursesByTitle(CategoryTitleConstants.CATEGORY_DIALOG_TITLE)
        ...
        /*coursePreferences.courses.asLiveData().observe(requireActivity()){
            if(it != null){
                ...
                // Логика загрузки изображений каждого курса
                // с сохранением в локальном хранилище
                val dataImages = runBlocking {
                    coursePreferences.coursesImg.first()
                }

                if(dataImages == null){
                    // Нет изображений, необходимо скачать все изображения курсов
                    for(value in data.courses){
                        viewModel.getCourseTitleImage(value.id)
                    }
                }else{
                    // Есть изображения, необходимо определить какие изображения нужно скачать
                    val dataImagesValue = Gson().fromJson(
                        dataImages,
                        CourseImgDataModel::class.java
                    )

                    for(value in data.courses){
                        if(!existsElementById(value.id, dataImagesValue)){
                            // Загрузка изображения, которого нет в локальном хранилище
                            viewModel.getCourseTitleImage(value.id)
                        }
                    }
                }
    ...
    override fun getFragmentBinding(
        ...
        container: ViewGroup?
    ) = FragmentCategoryDialogBinding.inflate(inflater, container, false)
